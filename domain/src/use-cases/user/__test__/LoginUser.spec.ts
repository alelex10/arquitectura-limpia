...
import { LoginUser, LoginUserDependencies, LoginUserDto } from "../login-user.use-case";
import { createInvalidDataError } from "../../../errors/errors";
import { User } from "../../../entities/user.entity";
import { mockUserRepository, UserRepositoryMock } from "../../../mocks/user-repository-mock";
...
describe("LoginUser use-case", () => {
	...
	test("With empty email, fail with 'Email is required'", async () => {
		const payload: LoginUserDto = { email: "", password: "123" };
	...
	test("With empty password, fail with 'Password is required'", async () => {
		const payload: LoginUserDto = { email: "aa@a.com", password: "" };
		const result = await LoginUser(dependencies, payload);
	...
	test("With non-existing email, fail with 'No user found for this email'", async () => {
		const payload: LoginUserDto = { email: "no@no.com", password: "123" };
		const result = await LoginUser(dependencies, payload);
	...
	test("With wrong password, fail with 'Password is incorrect'", async () => {
		const payload: LoginUserDto = { email: "a@a.com", password: "wrong" };
	...
	test("With valid credentials, return the user", async () => {
		const payload: LoginUserDto = { email: "a@a.com", password: "correct" };
		const result = await LoginUser(dependencies, payload);
