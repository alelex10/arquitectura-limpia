...
describe("UpdateDocument use-case", () => {
	it("fails if document not found", async () => {
		...
		const res = await UpdateDocument({ documents: docs, versions }, { documentId: "x", userId: "u1", content: "new" });
	...
	it("fails if user not authorized", async () => {
		...
		const versions = mockVersionRepository([]);
		const res = await UpdateDocument({ documents: docs, versions }, { documentId: "d1", userId: "u1", content: "new" });
		expect(res).toEqual(createInvalidDataError("Not authorized to update document"));
	});
	...
	it("updates and creates new version when content changed", async () => {
		...
		const versions = mockVersionRepository([
			{ id: "v1", documentId: "d1", content: "old", version: 1, createdBy: "u1", createdAt: new Date() },
		]);
		const res = await UpdateDocument({ documents: docs, versions }, { documentId: "d1", userId: "u1", content: "new" });
